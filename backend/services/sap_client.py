...
class SAPClient:
    ...
    async def get_token(self) -> str:
        ...
        try:
            response = await self.client.post(settings.AUTH_URL, headers=headers, data=data)
    ...
    async def get_products(self, token: str) -> List[Dict[str, Any]]:
        ...
        if token == "mock_token":
            return [
                {"name": "INTENSIVE 12 Demo-Produkt", "bezeichnung": "INTENSIVE 12 Demo-Produkt", "basePrice": 120.00, "workingPrice": 32.5, "vertriebskanal": "Chatbot", "produktId": "INT12_DEMO_PROD"},
                {"name": "INTENSIVE 24 Demo-Produkt", "bezeichnung": "INTENSIVE 24 Demo-Produkt", "basePrice": 110.00, "workingPrice": 31.0, "vertriebskanal": "Chatbot", "produktId": "INT24_DEMO_PROD"},
                {"name": "Green Energy Eco", "bezeichnung": "Green Energy Eco", "basePrice": 140.00, "workingPrice": 34.5, "vertriebskanal": "Chatbot", "oeko": True, "produktId": "GREEN_ECO_PROD"}
            ]
        ...
        try:
            response = await self.client.get(settings.PRODUCT_URL, headers=headers)
            response.raise_for_status()
    ...
    async def simulate_price(self, token: str, consumption_r1: float, product_id: str, consumption_r2: str = "") -> Optional[Dict[str, Any]]:
        ...
        """
        if token == "mock_token":
            return {"total_price": 123.45, "currency": "EUR", "BillSimulation": {"NET_AMOUNT": 123.45}}
        ...
        try:
            response = await self.client.get(settings.SIMULATION_URL, headers=headers, params=payload)
            response.raise_for_status()
            ...
            try:
                response = await self.client.post(settings.SIMULATION_URL, headers=headers, json=payload)
                response.raise_for_status()
                sim_result = response.json()
                ...
                # httpx exceptions might not have 'response' attribute in the same way as requests
    ...
    async def create_offer(self, token: str, product_id: str, start_date: str, consumption_r1: float, consumption_r2: str = "", user_details: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            response = await self.client.post(settings.OFFER_URL, headers=headers, json=payload, timeout=10.0)
